...
// --- SHADERS (Unchanged logic, just keeping structure) ---
...
const Nebula = ({ theme }: { theme: ColorTheme }) => {
  ...
  // RICH COLOR PALETTE UPDATE
  const colors = useMemo(() => {
    if (theme === 'warm') return { a: new THREE.Color('#380e0e'), b: new THREE.Color('#4a2c00') }; // Deep Ember
    if (theme === 'cool') return { a: new THREE.Color('#031826'), b: new THREE.Color('#002422') }; // Deep Abyss
    return { a: new THREE.Color('#0f0f13'), b: new THREE.Color('#1a1a20') }; // Void
  }, [theme]);
...
function getThemeColors(theme: ColorTheme): { core: THREE.Color, rim: THREE.Color } {
  // UPGRADED PALETTE
  if (theme === 'warm') {
    // Rose Gold / Sunset
    return { core: new THREE.Color('#d14d36'), rim: new THREE.Color('#ffae6e') };
  }
  if (theme === 'cool') {
    // Bioluminescent Ocean
    return { core: new THREE.Color('#0b4f6e'), rim: new THREE.Color('#3cebe1') };
  }
  // Moonlight / Silver
  return { core: new THREE.Color('#5e5e6e'), rim: new THREE.Color('#a8a8bd') }; 
}

function resolveQuality(quality: QualityTier): { dpr: number; segments: number } {
  const baseDpr = Math.min(window.devicePixelRatio || 1, 2);
  if (quality === 'low') return { dpr: Math.min(baseDpr, 1), segments: 48 }; 
  if (quality === 'medium') return { dpr: Math.min(baseDpr, 1.5), segments: 72 };
  return { dpr: Math.min(baseDpr, 2.0), segments: 128 }; 
}
